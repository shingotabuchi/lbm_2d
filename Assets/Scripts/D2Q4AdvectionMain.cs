...
public class D2Q4AdvectionMain : MonoBehaviour
{
    ...
    float dx = 1f;
    float dt = 1f;
    float csq,omega,feq1,feq2,feq3,feq4,ck;
    void Start()
    {
    ...
    void Update()
    {
        ...
        for (int i = 0; i < resInt; i++)
        {
            for (int j = 0; j < resInt; j++)
            {
                feq1 = 0.25f*rho[i,j]*(1f + 2f*u/ck);
                feq2 = 0.25f*rho[i,j]*(1f - 2f*u/ck);
                feq3 = 0.25f*rho[i,j]*(1f + 2f*v/ck);
                feq4 = 0.25f*rho[i,j]*(1f - 2f*v/ck);
                f1[i,j] = omega*feq1 + (1f - omega)*f1[i,j];
                f2[i,j] = omega*feq2 + (1f - omega)*f2[i,j];
                f3[i,j] = omega*feq3 + (1f - omega)*f3[i,j];
                f4[i,j] = omega*feq4 + (1f - omega)*f4[i,j];
            }
        }
        // Streaming Process
        for (int j = 0; j < resInt; j++)
